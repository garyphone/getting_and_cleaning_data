...
# Result 3
msg("The value of sum(datZip∗datExt,na.rm=T) is", answer3)
...
#
# 1. sapply(split(DTpwgtp15,DTSEX),mean)
# 2. tapply(DTpwgtp15,DTSEX,mean)
# 3. mean(DTpwgtp15,by=DTSEX)
# 4. DT[,mean(pwgtp15),by=SEX]
# 5. rowMeans(DT)[DTSEX==1];rowMeans(DT)[DTSEX==2]
# 6. mean(DT[DTSEX==1,]pwgtp15); mean(DT[DTSEX==2,]pwgtp15)
...
# Answer: The function 'mean(DTpwgtp15,by=DTSEX)' should be the fastest one.
